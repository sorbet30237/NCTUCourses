...
//		write(1,"A",sizeof("A"));
	...
	struct block *bestpointer;
	int bestsize = 30000;
	while(1){
		if(current->size >= nbytes && current->free == 1){
	...
	if(bestpointer == NULL){
		...
		struct block *temp;
		temp = (bestpointer+(bestpointer->size+32)/32)-(32+nbytes)/32;
		if(temp == bestpointer){
			bestpointer->free = 0;
			return (void*)bestpointer+32;
		}
		temp->size = nbytes;
		if(bestpointer->next != NULL){
			temp->next = bestpointer->next;
			bestpointer->next->prev = temp;
			bestpointer->next = temp;
			temp->prev = bestpointer;
		}
		else{
			bestpointer->next = temp;
			temp->prev = bestpointer;
		...
		temp->free = 0;
		if(temp != bestpointer){
			bestpointer->size = bestpointer->size - 32 - nbytes;
		}
		char msg[100];
		memset(msg, 0, 100);
//		sprintf(msg, "\n%d %d\n", bestpointer,bestpointer->next);
